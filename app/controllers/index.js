...
function init(){
	setAddTaskButton();
	$.index.open();
	getToDo(); //1. put code here to populate the table for the toDo list
	getDone();
}
...
function setAddTaskButton(){
	...
	btnAddTask.addEventListener('click',function(){addTask();}); //2. add an event listener for the btnAddTask button for the add task function
	...
	dialog.addEventListener('click', function(dialog_evt) {
		if(dialog_evt.index === 0){
			if(dialog_evt.text.trim().length > 0){
				saveTask(dialog_evt.text);//4. add code here to save the task
				getToDo();
			}else{
...
function getToDo(){
	...
	for(var i=0; i<tasksCollection.length; i++){
		...
		row.addEventListener('click',function(e){ todoRowFunction(e); });//5. add an event listener for the row for the click event
...
function getDone(){
	...
	for(var i=0; i<tasksCollection.length; i++){
		...
		var row = Titanium.UI.createTableViewRow({
			'title'		: title,
			'c_id'		: id,//6. something is missing here, what is it?
			'hasCheck'	: true
		});
...
function todoRowFunction(row_evt){
	...
	dialog.addEventListener('click',function(dialog_evt){
		if(dialog_evt.index === 0){
			setItemAsDone(row_evt.source.c_id);//8. add function here to edit/update the task as done 
			getToDo();
			getDone();
		}
	});
...
function doneRowFunction(row_evt){
	...
	dialog.addEventListener('click',function(dialog_evt){
		if(dialog_evt.index === 0){
			...
			getDone();//9. add function here to refresh the done table list
		}
	});
	dialog.show();
...
function setItemAsDone(id){
	...
	if(tasksCollection.length>0){
		var model = tasksCollection.at(0);//10. declare a variable named "mode" here from the first item of the tasksCollection
		model.set({
			'status': 1
		}).save();
	}
